...
if st.session_state.current_page == "onboarding":
    ...
    with col2:
        ...
        with st.container(border=True):
            with st.form("char"):
                st.session_state.name = st.text_input("Adventurer Name", value="Arpit Singh Sandha")
...
# --- VIEW 3: THE BATTLE ARENA ---
    ...
    if quest['category'] == "Coding":
        ...
        with right:
            with st.container(border=True):
                with st.form("code_execution_form", clear_on_submit=False):
                    ...
                    submit = st.form_submit_button("⚡ CAST SPELL (Execute Code)", type="primary")

                    if submit and not st.session_state.execution_in_progress:
                        st.session_state.execution_in_progress = True
                        st.session_state.user_code = editor
                        
                        try:
                            # 1. Local Pre-check
                            local_env = {}
                            exec(editor, local_env)
                            ...
                            if not passed_local:
                                ...
                                with st.spinner("The AI Judge is testing hidden edge cases and time complexity..."):
                                    ai_feedback = evaluate_submission(editor, prob.get("description", ""))
